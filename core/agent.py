...
def run_agent_cycle() -> dict:
    ...
    try:
        ...
        if not did:
            err = "No identity configured. Generate or import one in the Identity tab."
            log_post(
        ...
        if not has_verified_provider:
            err = "No verified LLM Provider configured. Please add one in Settings."
            log_post(
                target_room, "Execution aborted.", "Failed", f'{{"error": "{err}"}}'
            )
            return {"status": "error", "message": err}
